...
wait(int *status)
{
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      havekids = 1;
      if(p->state == ZOMBIE){
      // if status is not NULL, pass the child's status (pid)
      // to the parent before it gets set to 0 
      if (status) {
	*status = p->status;
      }
...
{
  /*
 *  To turn this round-robin into a priority driven round robin,
 *  we need to:
 *  	- find highest priority
 *  	- keep track of it
 *  	- ONLY run the runnable procs WITH the highest proc
 *  	- find the next highest and restart that
 */
  ...
  for(;;){
    ...
    // find matching priority thats runnable
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if(p->state != RUNNABLE)
        continue;

      if(p->priority != highestPriority){
	//cprintf("\nPROCESS# %d HAS AGED, new1 priority=%d!\n", p->pid, p->priority);
        if (p->priority > 0) {
      ...
      c->proc = 0;
      //p->priority = p->priority + 1; // reduce priority as process gets its turn to run
    }
